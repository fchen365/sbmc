...
getCascClusters = function(graphMat, covariates, nBlocks,
  ...
  if ((enhancedTuning == T) & (nSubspaces > 1)) {
    ...
    for (i in 1:nSubspaces) {
      subMinIndex[i] = which.min(wcssVec[subspaces$subintervalStart[i]:subspaces$subintervalEnd[i]]) +
        subspaces$subintervalStart[i] - 1
      subMaxIndex[i] = which.max(wcssVec[subspaces$subintervalStart[i]:subspaces$subintervalEnd[i]]) +
        subspaces$subintervalStart[i] - 1
    }
    ...
    minCountSubspaces = ((1:nSubspaces)[includeVec == 1])[which.min(subspaces$orthoCounts[includeVec == 1])]
...
getCascSvd = function(graphMat, covariates, hTuningParam, nBlocks) {
  ...
  singDecomp = irlbaMod(
    list(
      graphMat = graphMat, covariates = covariates,
      hTuningParam = hTuningParam
    ), nu = nBlocks + 1, nv = 0,
    m_b = internalDim, matmul = matrixMulti
  )
  
  return(list(
    singVec = singDecomp$u[, 1:nBlocks],
...
getTuningRange = function(graphMatrix, covariates, nBlocks) {
  ...
  hmin = (singValGraph[nBlocks] - singValGraph[nBlocks + 1]) / singValCov[1] ^
    2
